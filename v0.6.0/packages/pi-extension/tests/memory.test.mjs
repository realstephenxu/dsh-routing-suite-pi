...
test("distillTopic removes superseded and truncates", () => {
  const dir = mkdtempSync(join(tmpdir(), "dsh-memory-"));
  const file = join(dir, "memory.json");
  try {
    const store = memoryMod.loadStore(file);
    const session = memoryMod.getSessionMemory(store, "s1", "/tmp/project");
    const topic = memoryMod.ensureActiveTopic(session, "修复登录超时");
    const e1 = memoryMod.addMemoryEntry(session, topic, "decision", "决策A", 5);
    const e2 = memoryMod.addMemoryEntry(session, topic, "decision", "决策B", 5);
    memoryMod.markEntrySuperseded(session, topic, e1.id, e2.id);
    memoryMod.distillTopic(topic, { maxDecisions: 1, maxDesigns: 1, maxErrors: 1, maxTodos: 1, maxSummaryChars: 500 });
    assert.equal(topic.decisions.length, 1);
    assert.equal(topic.decisions[0].id, e2.id);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("distillSession creates lastSummary", () => {
  const dir = mkdtempSync(join(tmpdir(), "dsh-memory-"));
  const file = join(dir, "memory.json");
  try {
    const store = memoryMod.loadStore(file);
    ...
    const topic = memoryMod.ensureActiveTopic(session, "修复登录超时");
    memoryMod.addMemoryEntry(session, topic, "decision", "使用 Redis 分布式锁", 7);
    const summary = memoryMod.distillSession(session);
    assert.match(summary, /修复登录超时/);
    assert.match(summary, /使用 Redis 分布式锁/);
    assert.equal(session.lastSummary, summary);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("compaction summary includes distilled memory", async () => {
  const pi = createMockPi({ "dsh-memory": true });
  dshRoutingPi(pi);
  const ctx = makeCtx("s1");
  await pi._commands.get("dsh-memory-note").handler("修复登录超时：使用 Redis 分布式锁", ctx);
  const handler = pi._handlers.get("session_before_compact");
  const result = await handler(
...
test("session_shutdown distills memory", async () => {
  const pi = createMockPi({ "dsh-memory": true });
  dshRoutingPi(pi);
  const ctx = makeCtx("s1");
  await pi._commands.get("dsh-memory-note").handler("修复登录超时：使用 Redis 分布式锁", ctx);
  const handler = pi._handlers.get("session_shutdown");
  assert.ok(handler, "session_shutdown handler is registered");
  await handler({}, ctx);
  const summaryCmd = pi._commands.get("dsh-memory-summary");
  const ctx2 = makeCtx("s1");
  await summaryCmd.handler(undefined, ctx2);
  assert.ok(ctx2._notifications.some((n) => /使用 Redis 分布式锁/.test(n.message)));
});
