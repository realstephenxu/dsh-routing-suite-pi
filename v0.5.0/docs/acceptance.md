...
Current status: **PASS** (v0.3.0 core + extension + memory tests + Pi live stress with DeepSeek V4 Flash).
...
- Baseline: route 21/29, scope 24/29, convergence 29/29
- Routed score 87 >= baseline score 74
- Artifact: `artifacts/pi-live-stress-final.json`

## Memory (v0.3)

- `--dsh-memory` enables session memory.
- `/dsh-memory`, `/dsh-memory-note`, `/dsh-memory-forget`, `/dsh-memory-config` commands.
- Memory is stored at `~/.pi/agent/dsh-memory-v0.3.json`.
