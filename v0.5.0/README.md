...
Version 0.5.0 implemented, compiled, and unit tested. Memory distillation and compaction optimization added.

Design: `docs/v0.5-compaction-distillation-design.md` and `PLAN.md`.
...
- `/dsh-status` shows the current routing state.
- `/dsh-route <plan|inspect|fix|build|adaptive|weak|auto>` sets/clears a route override.
- `--dsh-two-phase` enables the optional two-phase tool surface.
- `--dsh-memory` enables session memory and context re-injection.
