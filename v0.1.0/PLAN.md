...
> 当前阶段：编译前设计。以下为后续实施顺序，尚未开始写代码/编译。
...
- [x] Schema 草案
- [ ] 确认配置默认值
- [ ] 确认 `weak` vs `adaptive` fallback 策略

## Phase 1 — Core 更新

- [ ] 更新 `routing-rules.json`：
  - 增加 `engineering` 信号
...
- [ ] 新增 `router-config` 读取/默认值
- [ ] 实现两阶段工具面：
  - `before_agent_start` 设置 minimal tools
  - 首次 `tool_call` 后展开 full tools
- [ ] 实现 `/dsh-status`
- [ ] 实现 `/dsh-route <route|auto>`
- [ ] 集成 Plan Mode 优先级
- [ ] 更新 Pi extension tests

## Phase 3 — 文档与 Schema 完善

- [ ] 更新 `docs/design.md`
- [ ] 更新 `docs/acceptance.md`
- [ ] 新增 `packages/pi-extension/schemas/router-config.schema.json`
- [ ] 新增 `packages/pi-extension/schemas/route-override.schema.json`
- [ ] 新增 `packages/pi-extension/schemas/tool-phase.schema.json`

## Phase 4 — 编译与测试

- [ ] `npm run build`
- [ ] `npm test`
- [ ] Pi live stress（DeepSeek V4 Flash）
- [ ] 生成最终 artifact
